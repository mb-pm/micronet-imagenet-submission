...
from quant_layers import Conv2D, DepthwiseConv2D, BinConv2D
...
class MBConvBlockBinary(tf.keras.layers.Layer):
  """A class of MBConvBinary: Mobile Inverted Residual Bottleneck.
  ...
  def __init__(self, block_args, global_params):
    """Initializes a MBConvBinary block.

    Args:
      block_args: BlockArgs, arguments to create a Block.
      global_params: GlobalParams, a set of global parameters.
    """
    super(MBConvBlockBinary, self).__init__()
    self._block_args = block_args
    self._batch_norm_momentum = global_params.batch_norm_momentum
    self._batch_norm_epsilon = global_params.batch_norm_epsilon
    self._data_format = global_params.data_format
  ...
  def _build(self):
    ...
    self._project_conv = BinConv2D(
        ...
        quantize=True,
        quantize_bits=7,
        min_deviation_multiplier=2,
        max_deviation_multiplier=40
    )
    self._bn2 = batchnorm(
        axis=self._channel_axis,
        momentum=self._batch_norm_momentum,
  ...
  def _call_se(self, input_tensor, training):
    """Call Squeeze and Excitation layer.
    ...
    """
    se_tensor = tf.reduce_mean(input_tensor, self._spatial_dims, keepdims=True)
    se_tensor = self._se_expand(self._relu_fn(self._se_reduce(se_tensor, training=training)), training=training)
    tf.logging.info('Built Squeeze and Excitation with tensor shape: %s' %
                    (se_tensor.shape))
    return tf.sigmoid(se_tensor) * input_tensor
  ...
  def call(self, inputs, training=True, drop_connect_rate=None):
    ...
    Returns:
      A output tensor.
    """
    tf.logging.info('>' * 50 + 'MBConvBlockBinary call.')
    tf.logging.info('Block input: %s shape: %s' % (inputs.name, inputs.shape))
    if self._block_args.expand_ratio != 1:
      x = self._relu_fn(self._bn0(self._expand_conv(inputs, training=training), training=training))
    else:
      x = inputs
    tf.logging.info('Expand: %s shape: %s' % (x.name, x.shape))
    ...
    if self._has_se:
      with tf.variable_scope('se'):
        x = self._call_se(x, training)

    self.endpoints = {'expansion_output': x}

    x = self._bn2(self._project_conv(x, training=training), training=training)
    if self._block_args.id_skip:
      if all(
          s == 1 for s in self._block_args.strides
      ) and self._block_args.input_filters == self._block_args.output_filters:
...
class Model(tf.keras.Model):
  ...
  def _get_conv_block(self, conv_type):
    conv_block_map = {0: MBConvBlockBinary, 2: MBConvBlock}
  ...
  def _build(self):
    ...
    self._conv_head = Conv2D(
        filters=self._global_params.num_classes,#round_filters(1280, self._global_params),
        kernel_size=[1, 1],
        strides=[1, 1],
        kernel_initializer=conv_kernel_initializer,
        padding='same',
