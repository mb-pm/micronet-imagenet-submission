...
def model_fn(features, labels, mode, params):
  ...
  if is_training:
    ...
    #quant_ops = [op for op in tf.get_default_graph().get_operations() if 'quantization_batchnorm' in op.name and op.type == 'AssignSubVariableOp']
    #update_ops.extend(quant_ops)
  ...
  return tfe.estimator.tpu.TPUEstimatorSpec(
      mode=mode,
      loss=loss,
      train_op=train_op,
      host_call=host_call,
...
def main(unused_argv):
  ...
  est = tfe.estimator.tpu.TPUEstimator(
