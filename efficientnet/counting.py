...
Conv2D = collections.namedtuple(
    'Conv2D', ['input_size', 'kernel_shape', 'strides', 'padding', 'use_bias',
               'activation'])
Conv2DBinary = collections.namedtuple(
    'Conv2DBinary', ['input_size', 'kernel_shape', 'strides', 'padding', 'use_bias',
               'activation'])
...
def count_ops(op, sparsity, param_bits, add_bits, base_add_bits, mul_bits, base_mul_bits):
  ...
  if isinstance(op, Conv2D):
  ...
  elif isinstance(op, Conv2DBinary):
    # Square kernel expected.
    assert op.kernel_shape[0] == op.kernel_shape[1]
    k_size, _, c_in, c_out = op.kernel_shape
    ...
    # Size of the possibly sparse convolutional tensor.
    param_count += get_sparse_size(
        [k_size, k_size, c_in, c_out], 1, sparsity)
  ...
  return (
      param_count,
      flop_mults * (mul_bits / base_mul_bits),
      flop_adds * (add_bits / base_add_bits),
  )
...
def get_info(op):
  ...
  if isinstance(op, (DepthWiseConv2D, Conv2D, Conv2DBinary)):
...
class MicroNetCounter(object):
  ...
  """
  _header_str = '{:25} {:>10} {:>13} {:>13} {:>13} {:>15} {:>15} {:>15} {:>15}'
  _line_str = ('{:25s} {:10d} {:13d} {:13d} {:13d} {:15.3f} {:15.3f}'
               ' {:15.3f} {:15.3f}')
  ...
  def process_counts(self, total_params, total_mults, total_adds,
    ...
    total_mults = total_mults * mul_bits / self.mul_bits_base / 1e9
    total_adds = total_adds * add_bits / self.add_bits_base / 1e9
  ...
  def _print_header(self):
    output_string = self._header_str.format(
        ...
        'params(MBytes)', 'mults(G)', 'adds(G)', 'GFLOPS')
    print(output_string)
    print(''.join(['=']*125))
  ...
  def _print_line(self, name, input_size, kernel_size, in_channels,
    ...
    op_pc = param_count / 8. / 1e6
    op_mu = flop_mults / 1e9
    op_ad = flop_adds / 1e9
  ...
  def print_summary(self, sparsity, param_bits, add_bits, mul_bits,
    ...
    def get_add_bits(block, op):
        if get_block_index(block) >= 2 and op in qaunt_bits.keys():
            return qaunt_bits[op]
        return add_bits

    def get_mul_bits(block, op):
        if get_block_index(block) >= 2 and op in qaunt_bits.keys():
            return 1
        return add_bits
      ...
      if op_name.startswith('block'):
        if not summarize_blocks:
          ...
          for block_op_name, block_op_template in op_template:
            param_count, flop_mults, flop_adds = count_ops(block_op_template,
                                                           sparsity, param_bits,
                                                           add_bits=get_add_bits(op_name, block_op_name),
                                                           mul_bits=get_mul_bits(op_name, block_op_name),
                                                           base_add_bits=self.add_bits_base,
                                                           base_mul_bits=self.mul_bits_base)
        ...
        param_count, flop_mults, flop_adds = self._aggregate_list(
            [count_ops(template, sparsity, param_bits,
                       ...
                       base_add_bits=self.add_bits_base,
                       base_mul_bits=self.mul_bits_base)
             for block_op_name, template in op_template])
    ...
    out_str = ('{:25s} {:10s} {:13s} {:13s} {:13s} {:15.3f} {:15.3f} {:15.3f} '
               '{:15.3f}')
    self._print_line(
        'total', '', '', '', '', total_params, total_mults, total_adds,
        mul_bits, add_bits, base_str=out_str)
    print('Total params (bits):', total_params)
